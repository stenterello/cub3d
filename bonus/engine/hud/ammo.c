...
/*   Updated: 2022/10/01 19:08:25 by ddelladi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_ammo_amount(t_rules *rules)
{
	...
	mlx_string_put(rules->mlx.mlx, rules->mlx.mlx_win, rules->mlx.win_width - 55, 60, 0x00FFFFFF, s);
	free(s);
}
...
void	ammo_level(t_rules *rules, t_image *view)
{
	...
	while (info.start_x < info.end_x)
	{
		...
		info.t_x = (info.start_x - info.bench_x) * rules->player.ammo_tex.width / info.width;
		while (info.start_y < info.end_y)
		{
			info.t_y = (info.start_y - info.bench_y) * rules->player.ammo_tex.height / info.height;
			info.color = get_sprite_color(&rules->player.ammo_tex, info.t_x, info.t_y, rules);
			if (info.color)
